...
/-- Choose a representative of `x : Grassmannian K V I` in `I → V`. -/
protected noncomputable def QGrassmannian.rep (x : QGrassmannian K V I) : I → V :=
  x.out'

theorem QGrassmannian.rep_linearIndependent (x : QGrassmannian K V I) : LinearIndependent K x.rep :=
  x.out'.2
...
lemma MatrixAction_vs_comp (f : (I → K) →ₗ[K] (I → K)) (v w : I → V) :
  ...
  unfold MatrixAction
  constructor
  . exact fun h ↦ by rw [h]; simp only [LinearEquiv.invFun_eq_symm, LinearEquiv.apply_symm_apply]
  . intro h
...
def MatrixAction_inv (f : (I → K) ≃ₗ[K] (I → K)) (v w : I → V) :
  ...
  constructor
  . exact fun h ↦ by rw [h, ← MatrixAction_mul]; simp only [LinearEquiv.comp_coe,
      LinearEquiv.symm_trans_self, LinearEquiv.refl_toLinearMap, MatrixAction_id]
  . exact fun h ↦ by rw [h, ← MatrixAction_mul]; simp only [LinearEquiv.comp_coe,
      LinearEquiv.self_trans_symm, LinearEquiv.refl_toLinearMap, MatrixAction_id]

lemma MatrixAction_apply (f : (I → K) →ₗ[K] (I → K)) (v : I → V) (i : I) :
    MatrixAction K f v i = Finset.sum ⊤ (fun j => (f (Pi.basisFun K I i) j) • v j) := by
  unfold MatrixAction
  rw [← (Basis.constr_basis (Pi.basisFun K I) ℤ ((Basis.constr (M' := V) (Pi.basisFun K I) ℤ).invFun
            ((Basis.constr (Pi.basisFun K I) ℤ  v).comp f)) i)]
  simp only [LinearEquiv.invFun_eq_symm, LinearEquiv.apply_symm_apply, Pi.basisFun_apply,
    LinearMap.coe_comp, Function.comp_apply, Basis.constr_apply_fintype, Pi.basisFun_equivFun,
    LinearEquiv.refl_apply, Finset.top_eq_univ]
...
Submodule.span K (Set.range (MatrixAction K f v)) ≤ Submodule.span K (Set.range v) := by
  ...
  exact fun _ heq ↦ by rw [← heq, MatrixAction_apply]
...
lemma MatrixAction_vs_SubmoduleSpan (v w : I → V) :
  ...
  constructor
  . intro h
    have hsub : LinearMap.range (Basis.constr (Pi.basisFun K I) ℤ v) ≤
        LinearMap.range (Basis.constr (Pi.basisFun K I) ℤ w) := by
      rw [Basis.constr_range, Basis.constr_range]
      exact h
    set f := (Basis.constr (Pi.basisFun K I) ℤ v).codRestrict (LinearMap.range (Basis.constr
      (Pi.basisFun K I) ℤ w)) (by intro u; apply hsub; simp only [Basis.constr_apply_fintype,
      Pi.basisFun_equivFun, LinearEquiv.refl_apply, LinearMap.mem_range, exists_apply_eq_apply])
    set g := (Basis.constr (Pi.basisFun K I) ℤ w).rangeRestrict
    have hsur : Function.Surjective g := by rw [←LinearMap.range_eq_top]
                                            apply LinearMap.range_rangeRestrict
    obtain ⟨h, prop⟩ := Module.projective_lifting_property g f hsur
    existsi h
    simp_rw [MatrixAction_vs_comp]
    have heq : (Basis.constr (Pi.basisFun K I) ℤ w)  = (Submodule.subtype (LinearMap.range
        ((Basis.constr (Pi.basisFun K I) ℤ w)))).comp g := by
      simp only [LinearMap.subtype_comp_codRestrict]
    rw [heq, LinearMap.comp_assoc, prop]
    simp only [LinearMap.subtype_comp_codRestrict]
  . exact fun h ↦ let ⟨_, hf⟩ := h; by rw [hf]; apply MatrixAction_span
...
lemma MatrixAction_uniqueness {v : I → V} (hv : LinearIndependent K v)
  ...
  simp only [LinearEquiv.invFun_eq_symm, EmbeddingLike.apply_eq_iff_eq] at heq
  apply_fun (fun h => h u) at heq
  simp only [LinearMap.coe_comp, Function.comp_apply] at heq
  exact LinearMap.ker_eq_bot.mp (Basis.constr_ker _ _ hv) heq
...
lemma QGrassmannian.mk_eq_mk_iff_matrixAction (v w : I → V) (hv : LinearIndependent K v)
  ...
  . intro heq
    ...
    have hgf : LinearMap.comp g f = LinearMap.id := by
      ...
      conv_lhs at hf => rw [← (MatrixAction_id K v)]
      exact Eq.symm (MatrixAction_uniqueness K hv _ _ hf)
    have hfg : LinearMap.comp f g = LinearMap.id := by
      rw [hf, ←MatrixAction_mul] at hg
      conv_lhs at hg => rw [←(MatrixAction_id K w)]
      exact Eq.symm (MatrixAction_uniqueness K hw _ _ hg)
...
lemma QGrassmannian.exists_matrixAction_eq_mk_rep (v : I → V) (hv : LinearIndependent K v) :
    ∃ (f : (I → K) ≃ₗ[K] (I → K)), MatrixAction K f v = QGrassmannian.rep
    (QGrassmannian.mk K v hv) := by
...
@[elab_as_elim]
lemma QGrassmannian.ind {P : QGrassmannian K V I → Prop} (h : ∀ (v : I → V)
    (h : LinearIndependent K v), P (QGrassmannian.mk K v h)) : ∀ p, P p :=
...
lemma QGrassmannian.submodule_eq (x : QGrassmannian K V I) :
    x.submodule = Submodule.span K (Set.range x.rep) := by
  conv_lhs => rw [← x.mk_rep]

instance QGrassmannian.finiteDimensional_submodule (x : QGrassmannian K V I) :
    FiniteDimensional K x.submodule := by
  rw [QGrassmannian.submodule_eq]
  exact FiniteDimensional.span_of_finite _ (Set.finite_range _)

lemma QGrassmannian.finrank_submodule (x : QGrassmannian K V I) :
    FiniteDimensional.finrank K x.submodule = Fintype.card I := by
  rw [QGrassmannian.submodule_eq, finrank_span_eq_card (QGrassmannian.rep_linearIndependent x)]

variable {r}

lemma QGrassmannian.finrank_submodule' (x : QGrassmannian K V I) :
    FiniteDimensional.finrank K x.submodule = r := by
  rw [QGrassmannian.submodule_eq, finrank_span_eq_card (QGrassmannian.rep_linearIndependent x)]
  exact hrI
...
def QGrassmannianToGrassmannian : QGrassmannian K V I → Grassmannian K V r :=
  ...
  QGrassmannian.finrank_submodule' hrI x⟩⟩

lemma QGrassmannianToGrassmannian_apply' {v : I → V} (hv : LinearIndependent K v) :
    (QGrassmannianToGrassmannian K V hrI (QGrassmannian.mk K v hv)).1 =
...
lemma QGrassmannianToGrassmannian_apply (x : QGrassmannian K V I) :
  ...
  conv_lhs => rw [←(QGrassmannian.mk_rep x)]
...
def GrassmannianToQGrassmannian : Grassmannian K V r → QGrassmannian K V I := by
  ...
  set B :=  (Basis.reindex (FiniteDimensional.finBasisOfFinrankEq K W W.2.2)
    (Fintype.equivOfCardEq heq)) -- somehow this avoids a deterministic timeout
  exact QGrassmannian.mk K ((Submodule.subtype W.1) ∘ (fun i => B i))
    (LinearIndependent.map' (Basis.linearIndependent B) _ (Submodule.ker_subtype W.1))
...
GrassmannianToQGrassmannian K V hrI (QGrassmannianToGrassmannian K V hrI x) = x := by
  ...
  conv_rhs => rw [← (QGrassmannian.mk_rep x)]
  rw [QGrassmannian.mk_eq_mk_iff_span, Set.range_comp]
  conv_lhs => erw [← (LinearMap.map_span (Submodule.subtype _))]
...
noncomputable def QGrassmannianEquivGrassmannian : QGrassmannian K V I ≃ Grassmannian K V r where
...
/- Since we have equivalence, we can define Grassmannian.mk and Grassmannian.rep by composing the
QGrassmannian versions with the equivalence.-/
...
theorem Grassmannian.mkI_repI (x : Grassmannian K V r) :
    Grassmannian.mkI K hrI (Grassmannian.repI hrI x)
    (Grassmannian.repI_linearIndependent hrI x) = x := by
...
lemma Grassmannian.mkI_eq_mkI_iff_matrixAction (v w : I → V) (hv : LinearIndependent K v)
    (hw : LinearIndependent K w) : Grassmannian.mkI K hrI v hv = Grassmannian.mkI K hrI w hw ↔
    ∃ (f : (I → K) ≃ₗ[K] (I → K)), w = MatrixAction K f v := by
  unfold Grassmannian.mkI
  simp only [EmbeddingLike.apply_eq_iff_eq]
  rw [QGrassmannian.mk_eq_mk_iff_matrixAction]

lemma Grassmannian.mk_eq_mk_iff_matrixAction (v w : Fin r → V) (hv : LinearIndependent K v)
    (hw : LinearIndependent K w) : Grassmannian.mk K v hv = Grassmannian.mk K w hw ↔
    ∃ (f : (Fin r → K) ≃ₗ[K] (Fin r → K)), w = MatrixAction K f v :=
...
lemma Grassmannian.exists_matrixAction_eq_mkI_repI (v : I → V) (hv : LinearIndependent K v) :
    ∃ (f : (I → K) ≃ₗ[K] (I → K)), MatrixAction K f v = Grassmannian.repI hrI
    (Grassmannian.mkI K hrI v hv) := by
...
lemma Grassmannian.exists_matrixAction_eq_mk_rep (v : Fin r → V) (hv : LinearIndependent K v) :
    ∃ (f : (Fin r → K) ≃ₗ[K] (Fin r → K)), MatrixAction K f v = Grassmannian.rep
    (Grassmannian.mk K v hv) :=
...
noncomputable def Projectivization.equivGrassmannian : ℙ K V ≃ Grassmannian K V 1 :=
  Equiv.trans (Projectivization.equivSubmodule K V)
  (Grassmannian.equivSubmodule K V Nat.zero_lt_one).symm

variable {K V}
variable (r I) {V' : Type*} [AddCommGroup V'] [Module K V']

/-- An injective linear map of vector spaces induces a map on QGrassmannians. This is
less general than the version for projective spaces because LinearIndependent.map' requires
the two rings to be equal.-/
def QGrassmannian.map (f : V →ₗ[K] V') (hf : Function.Injective f) :
    QGrassmannian K V I → QGrassmannian K V' I :=
  Quotient.map' (fun v => ⟨f ∘ v.1, by rw [←LinearMap.ker_eq_bot] at hf; exact LinearIndependent.map' v.2 f hf⟩)
    (by rintro ⟨v, hv⟩ ⟨w, hw⟩ hvw
        change Submodule.span K _ = _ at hvw
        change Submodule.span K _ = _
        simp only at hvw ⊢
        rw [Set.range_comp, Set.range_comp, ← LinearMap.map_span, ←LinearMap.map_span, hvw])

lemma QGrassmannian.map_mk (f : V →ₗ[K] V') (hf : Function.Injective f) (v : I → V)
    (hv : LinearIndependent K v) : QGrassmannian.map I f hf (mk K v hv) = QGrassmannian.mk K (f ∘ v)
    (by rw [←LinearMap.ker_eq_bot] at hf; exact LinearIndependent.map' hv f hf) := rfl

/-- The map `QGrassmannian.map f` is injective when it is defined. -/
theorem QGrassmannian.map_injective (f : V →ₗ[K] V') (hf : Function.Injective f) :
  ...
  rw [QGrassmannian.mk_eq_mk_iff_span]
  rw [QGrassmannian.map_mk, QGrassmannian.map_mk, QGrassmannian.mk_eq_mk_iff_span, Set.range_comp,
    Set.range_comp, ← LinearMap.map_span, ←LinearMap.map_span] at hxy
...
@[simp]
lemma QGrassmannian.map_id : QGrassmannian.map I (LinearMap.id : V →ₗ[K] V)
    (LinearEquiv.refl K V).injective = id := by
  ext ⟨v⟩
  rfl

lemma QGrassmannian.map_comp {U : Type*} [AddCommGroup U] [Module K U] (f : V →ₗ[K] V')
    (hf : Function.Injective f) (g : V' →ₗ[K] U) (hg : Function.Injective g) :
    QGrassmannian.map I (g.comp f) (hg.comp hf) = QGrassmannian.map I g hg ∘
    QGrassmannian.map I f hf := by
  ext ⟨v⟩
  rfl

/- And the versions with Grassmannians.-/

def Grassmannian.map (f : V →ₗ[K] V') (hf : Function.Injective f) :
    Grassmannian K V r → Grassmannian K V' r := by
  intro W
  ...
  . set f' := f.domRestrict W.1
    have heq : Submodule.map f W = LinearMap.range f' := by
      ext u
      simp only [Submodule.mem_map, LinearMap.mem_range, LinearMap.domRestrict_apply,
...
@[simp]
lemma Grassmannian.map_id : Grassmannian.map r (LinearMap.id : V →ₗ[K] V)
    (LinearEquiv.refl K V).injective = id :=
  funext (fun _ ↦ by rw [id_eq, ←SetCoe.ext_iff, Grassmannian.map_apply, Submodule.map_id])

lemma Grassmannian.map_comp {U : Type*} [AddCommGroup U] [Module K U] (f : V →ₗ[K] V')
    (hf : Function.Injective f) (g : V' →ₗ[K] U) (hg : Function.Injective g) :
    Grassmannian.map r (g.comp f) (hg.comp hf) = Grassmannian.map r g hg ∘
    Grassmannian.map r f hf :=
  funext (fun _ ↦ by rw [Function.comp_apply, ←SetCoe.ext_iff, Grassmannian.map_apply,
    Grassmannian.map_apply, Grassmannian.map_apply, Submodule.map_comp])
...
lemma NonemptyGrassmannian_iff :
    Nonempty ({v : I → V // LinearIndependent K v}) ↔ Nonempty (Grassmannian K V r) := by
...
lemma NonemptyGrassmannian_iff' :
    Nonempty ({v : Fin r → V // LinearIndependent K v}) ↔ Nonempty (Grassmannian K V r) :=
NonemptyGrassmannian_iff K V (Fintype.card_fin r)

lemma NonemptyGrassmannian_of_finrank (hfinrank : r ≤ FiniteDimensional.finrank K V) :
    Nonempty (Grassmannian K V r) := by
  by_cases hr : r = 0
  . rw [hr]
    set W : Submodule K V := ⊥
...
variable {𝕜 E : Type*} [NontriviallyNormedField 𝕜] [NormedAddCommGroup E] [Module 𝕜 E]
[BoundedSMul 𝕜 E]
...
/- We equip the `Grassmannian` with the coinduced topology from the equivalence with the
`QGrassmannian`. Note that this is also an induced topology, see `Equiv.induced_symm` and
